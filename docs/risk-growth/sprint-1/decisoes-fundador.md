...
| PF-10 | **Verificar o domínio no Resend**: em *Domains → Add domain* informar `lessriskmoregrowth.com`; o Resend gera **3 registros DNS** (SPF, DKIM, DMARC) **exclusivos dessa conta** — copiar e colar no painel de DNS da Squarespace. **Item mais urgente do projeto pelo relógio**: até 24 h de propagação, e nada no time acelera isso | US-06 (entrega do card por e-mail) → sem isso a compra não entrega | **Hoje/amanhã cedo** |
