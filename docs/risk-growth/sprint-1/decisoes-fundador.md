...
| PF-08a | **Criar loja + produto no Lemon Squeezy** (US$ 29, DF-02) e entregar a **URL de checkout**. ⚠️ A URL que vai ao ar é a **live** — test mode aceita cartão e não liquida. Configurar o *redirect after purchase* para `https://lessriskmoregrowth.com/thanks.html`. Não enviar API key | RG-02 (2 linhas de config, commit `27b8986`) | **13/08** |
| PF-08b | **Submeter a ativação da loja para pagamentos reais** — na MESMA janela de PF-08a, usando a URL pública da Vercel se o DNS ainda não tiver propagado. É o relógio de terceiro mais longo do lançamento (1–3 dias úteis; submissão de sexta perde o fim de semana). Quebrada de PF-08 pelo PM em 12/08 para o relógio deles começar antes de o nosso terminar | L2 (transação real, Go/No-Go) | **13/08** |
| PF-14 | **Apontar o DNS do domínio na Squarespace para a hospedagem (Vercel)** — sem tocar em NADA da raiz que seja MX/SPF (checklist de não-quebra no inventário DNS acima). Propagação: planejar 48h; verificação por consulta ao DNS público (como em PF-10). Era texto solto dentro de PF-02; ganhou número e dono por ser o relógio mais longo sem rastreamento | L1 (site no ar no domínio) | **13/08** |
| PF-15 | **Decisões de tese na janela**: (1) o que o 11/11 lê — recomendação PM+PO: manter a data imutável lendo H2.2+apetite+teto, e fixar segunda data absoluta para churn/assinantes ancorada no go-live da assinatura (a aritmética não produz 2 coortes até 11/11); (2) antecipação de mídia dentro do mesmo teto de R$ 8k (ex.: R$ 1,5k/sem × 2 sem, depois R$ 500/sem) para o gate H2.2 abrir a tempo da P2-2. Detalhes em `roadmap.md` | Leitura de 11/11; P2-2 | **Até 17/08** |
...
| PF-11 | **API key do Resend → variável de ambiente da Vercel** (`RESEND_API_KEY`), pelo painel da Vercel. ⚠️ **Chave secreta: não colar no chat, em documento nem no repositório.** Quem tem a chave envia e-mail em nome do domínio. Se for exposta por engano, revogar no Resend e gerar outra — é rápido e sem custo. **RG-06 já construído (commit `88064ba`)**: com a chave na Vercel + redeploy, o envio liga sem mudança de código; sem ela, responde 503 honesto | RG-06 (`/api/send-card`) | **Aberta** |
| PF-12 | **Decidir o armazenamento da lista de e-mails (US-06 AC4)**: o MVP não tem banco, então hoje **nada é persistido do nosso lado** — a lista vive como tags no painel do Resend (paliativo da squad). Porém a Privacy Policy §3/§8 descreve cópia do card **armazenada com o registro de e-mail** e retenção "até você se descadastrar" — texto público descrevendo armazenamento que não existe. Opções: **(a)** adotar Resend Audiences (fundador cria a audience e entrega o ID — a promessa vira verdade); **(b)** ajustar a copy da §3/§8 para descrever a realidade (só o e-mail enviado, sem cópia nossa). Recomendação do PM: **(a)**, mantém a promessa e é ~10 min de painel | Coerência da Privacy Policy; descadastro one-click (AC5) | Antes do Go |
| PF-13 | **Aprovar textos novos voltados ao usuário** (escritos pela squad em inglês, tom da marca): frase pré-lançamento dos botões de compra, corpo do e-mail que entrega o card, página `thanks.html`, rótulo da checkbox de consentimento — além da copy do banner de cookies (DT-01) e da leitura final da política, já pendentes. **Um pacote só de leitura, ~15 min** | Go/No-Go | Antes do Go |
...
| DT-06 | Pós-leitura de 11/11 (backlog PO): páginas legais não emitem `page_view`; evento de taxa de aceite do banner; evento `feedback_ready` do RG-10 (decisão do PM em 12/08: resposta Yes/No lida no Resend até lá, contrato de eventos intocado na janela de leitura) | product-owner | Pós-11/11 |
| DT-08 | **Descadastro one-click por link** (URL HTTPS, RFC 8058) nos e-mails transacionais/lista — hoje o `List-Unsubscribe` é `mailto:` (herdado do RG-06), funcional porém manual (cai na caixa do fundador) e não-conformante para o botão nativo dos clientes de e-mail. A §4 da política foi emendada em 12/08 para descrever a realidade ("reply unsubscribe"); este item devolve o one-click de verdade | squad-produto | Pós-lançamento |
