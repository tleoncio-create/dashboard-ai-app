...
- [ ] **RG-10** (US-10) — Pós-compra: feedback de 1 pergunta + lista de espera — `squad-produto` — **Must** ⬆ (roadmap R-11: única fonte da H2.2) — **construído em 12/08 (commit `a84c806`), sob veredito do QA**. `waitlist_joined` no ar (fora dos 7 congelados, pré-autorizado); H2.2 no painel com quebra por trilha. **Decisão do PM**: AC2 fica PARCIAL — resposta Yes/No lida no painel do Resend, não no report.html, para não tocar o contrato de eventos na janela de leitura (evento `feedback_ready` → DT-06, pós-11/11). Latente até PF-11; Privacy Policy §1/§2 emendada junto (comentário anônimo + 4ª finalidade do e-mail) — entra no pacote PF-13
